...
#' @param ignore_case Ignore case while searching compound key
...
#' @export
get_elements_from_msp <- function(msp_path = NULL, compound_key = NULL, ignore_case = FALSE, load_on_memory = TRUE){
  ...
  if (is.na(ignore_case)){
    warning("The ignore_case should be TRUE/FALSE ")
    return (NULL)
  }
  ...
  if (identical(ignore_case, TRUE)){
    fixed_search <- FALSE
  } else {
    fixed_search <- TRUE
  }
  
  compound_key <- paste(compound_key, ":", sep = "")  
  if (identical(load_on_memory, TRUE)){  
    data_as_readlines <- readr::read_lines(msp_path)
    compound_key_list <- data_as_readlines[sapply(data_as_readlines, function(x) grepl(pattern = compound_key, x = x, ignore.case = ignore_case, fixed = fixed_search))]                                              
    ...
    while (length(one_line <- readLines(file_con, n = 1, warn = FALSE)) > 0){
      if(grepl(pattern = compound_key, x = one_line, ignore.case = ignore_case, fixed = fixed_search)){
